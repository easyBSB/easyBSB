...
async function fillUserForm(
  ...
  const pwdCtrl = page.locator(
    'input[data-e2e="easybsb-users-manage-password"]'
  );
...
test.describe("User is admin and can add new users", () => {
  ...
  test("should contain a list with default user easybsb", async ({
    page,
    request,
  }) => {
  ...
  test("logged in user should have access rights to add new users", async ({
    page,
    request,
  }) => {
    const token = await resolveSessionToken(request);
    ...
    const createUserButton = page.locator(
      'button[data-e2e="easybsb-user-add"]'
    );
    await Promise.all([
  ...
  test('New user should not be allowed to create new Users with role "write"', async ({
    page,
    request,
  }) => {
